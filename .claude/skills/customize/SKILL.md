...
| `src/channels/feishu.ts` | Feishu bot connection, WebSocket events, send/receive |
...
1. Create `src/channels/{name}.ts` implementing the `Channel` interface from `src/types.ts` (see `src/channels/feishu.ts` for reference)
...
2. Ask: "Should Telegram messages create separate conversation contexts, or share with Feishu groups?"
3. Create `src/channels/telegram.ts` implementing the `Channel` interface (see `src/channels/feishu.ts` for reference)
