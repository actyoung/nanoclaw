...
export class FeishuChannel implements Channel {
  ...
  private async handleMessage(data: FeishuMessageEvent): Promise<void> {
    ...
    // 处理语音消息转录
    let content: string;
    const isVoice = isVoiceMessage(message.message_type, message.content);

    if (isVoice) {
      logger.info({ messageId: message.message_id }, 'Detected voice message, transcribing...');
      const transcript = await transcribeFeishuVoiceMessage(
        message,
        FEISHU_APP_ID || '',
        FEISHU_APP_SECRET || '',
      );
      content = transcript || '[Voice Message - transcription unavailable]';
      logger.info({ messageId: message.message_id, content }, 'Voice message transcribed');
    } else {
      // 解析普通文本消息内容
      try {
    ...
    // 如果是语音消息，添加标记
    if (isVoice) {
      processedContent = `[Voice: ${processedContent}]`;
    }
