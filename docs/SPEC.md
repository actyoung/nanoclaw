...
1. Each channel skill adds a file to `src/channels/` (e.g. `feishu.ts`, `telegram.ts`) that calls `registerChannel()` at module load time:
...
See existing skills (`/add-feishu`, `/add-telegram`, `/add-slack`, `/add-discord`, `/add-gmail`) for the pattern.
...
│   ├── {channel}_main/             # Main control channel (e.g., feishu_main/)
...
│   ├── auth/                      # Feishu authentication state (if needed)
...
Folder names follow the convention `{channel}_{group-name}` (e.g., `feishu_family-chat`, `telegram_dev-team`). The main group has `isMain: true` set during registration.
...
1. User sends a message via any connected channel
   │
   ▼
2. Channel receives message (e.g. WebSocket for Feishu, Bot API for Telegram)
   │
   ▼
...
| Feishu Session | data/feishu/ | Auto-created via WebSocket |
...
| "Token expired" | Feishu session expired | Restart service to reconnect |
